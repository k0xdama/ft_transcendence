	...
	return (
		<>
			<nav className='sticky top-4 z-[100] mx-auto flex w-[calc(100%-2rem)] max-w-[1200px] items-center justify-between rounded-2xl border border-purple-dim bg-card/80 px-6 py-3 shadow-[0_12px_40px_rgba(10,4,20,0.35)] backdrop-blur-2xl'>
				<div className='text-white text-[1.5rem] uppercase tracking-[0.18em] hover:text-purple-hover'>
					<Link to="/">Triple</Link>
				</div>
				...
				<div className='flex gap-8 flex-row items-center'>
					{isAuthenticated() ? (
						...
						<>
							<Link to="/login">
								<div className='text-xs uppercase tracking-ui text-white/70 transition-colors hover:text-purple-pale'>Log in</div>
							</Link>
							<Link to="/register">
